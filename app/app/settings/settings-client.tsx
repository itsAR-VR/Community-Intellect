...
import type { IntegrationId, UserRole } from "@/lib/types"
...
export function SettingsClient({
  ...
  const surveysAutomationEnabled = Boolean(getNested(settings, "automation.surveys.enabled") ?? false)
  const surveysCadence = String(getNested(settings, "automation.surveys.cadence") ?? "weekly")
  const surveysMaxPerRun = Number(getNested(settings, "automation.surveys.maxPerRun") ?? 50)
  const programmingRemindersEnabled = Boolean(getNested(settings, "automation.programmingReminders.enabled") ?? false)
        ...
        <TabsContent value="automation" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Automated survey sends (records in Supabase; delivery integration comes later)</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between gap-4">
                <div className="min-w-0">
                  <Label>Enable survey autosend</Label>
                  <p className="text-sm text-muted-foreground">Runs daily via Vercel Cron; creates/updates surveys for active members</p>
                </div>
                <Switch
                  checked={surveysAutomationEnabled}
                  disabled={!canEditSettings}
                  onCheckedChange={(v) => setSettings((s) => setNested(s, "automation.surveys.enabled", v))}
                />
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  <Label>Cadence</Label>
                  <Select
                    value={surveysCadence}
                    onValueChange={(v) => setSettings((s) => setNested(s, "automation.surveys.cadence", v))}
                    disabled={!canEditSettings}
                  >
                    <SelectTrigger>
                      <SelectValue />
                ...
                <div className="space-y-2">
                  <Label>Max sends per run</Label>
                  <Input
                    type="number"
                    value={surveysMaxPerRun}
                    disabled={!canEditSettings}
                    onChange={(e) => setSettings((s) => setNested(s, "automation.surveys.maxPerRun", Number(e.target.value)))}
                  />
                  <p className="text-xs text-muted-foreground">Safety limit to prevent large bursts</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Programming reminders</CardTitle>
              <CardDescription>In-app notifications for upcoming sessions in the next 24 hours</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between gap-4">
                <div className="min-w-0">
                  <Label>Enable programming reminders</Label>
                  <p className="text-sm text-muted-foreground">Runs daily via Vercel Cron; creates tenant-wide notifications</p>
                </div>
                <Switch
                  checked={programmingRemindersEnabled}
                  disabled={!canEditSettings}
                  onCheckedChange={(v) => setSettings((s) => setNested(s, "automation.programmingReminders.enabled", v))}
                />
              </div>
            </CardContent>
          </Card>

          <Button disabled={isSaving || !canEditSettings} onClick={() => void saveSettings(settings)}>
            <Save className="mr-2 h-4 w-4" />
            {isSaving ? "Saving..." : "Save Automation"}
          </Button>

          {!canEditSettings ? (
            <div className="text-sm text-muted-foreground">You have read-only access. Ask an admin or community manager to update settings.</div>
          ) : null}
        </TabsContent>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between gap-4">
                ...
                <Switch
                  ...
                  disabled={!canEditSettings}
                  onCheckedChange={(v) => setSettings((s) => setNested(s, "autosend.enabled", v))}
                />
              </div>
