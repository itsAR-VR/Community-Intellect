...
import type { TenantId, PerkCategory, Member, Perk, PerkRecommendation, PerkPartnerApplication } from "@/lib/types"
...
export function PerksClient({
  ...
  applications: PerkPartnerApplication[]
}) {
  const router = useRouter()
  const { canEdit } = useAuth()

  const perks = allPerks.filter((p) => p.tenantId === tenantId && p.active)
  const memberIds = members.map((m) => m.id)
  const recommendations = allRecommendations.filter((r) => memberIds.includes(r.memberId) && !r.dismissed)

  const [createOpen, setCreateOpen] = React.useState(false)
  const [name, setName] = React.useState("")
  const [partnerName, setPartnerName] = React.useState("")
  const [category, setCategory] = React.useState<PerkCategory | "">("")
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., AWS Credits" />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Select value={category} onValueChange={(v) => setCategory(v as PerkCategory)}>
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                <Label>Value</Label>
                <Input value={value} onChange={(e) => setValue(e.target.value)} placeholder="e.g., $5,000 credits" />
              </div>
              <div className="space-y-2">
              ...
              <Button
                className="w-full"
                disabled={!canEdit || isSubmitting || !name || !partnerName || !category || !description}
                onClick={async () => {
                  setIsSubmitting(true)
                  try {
                    const res = await fetch("/app/api/perks/create", {
                      method: "POST",
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {recommendations
                ...
                .map((rec) => {
                  ...
                  const perk = perks.find((p) => p.id === rec.perkId) ?? allPerks.find((p) => p.id === rec.perkId)
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            onClick={async () => {
                              setIsSubmitting(true)
                              try {
                                const res = await fetch("/app/api/perks/recommendations/mark-delivered", {
                                  method: "POST",
                                  headers: { "content-type": "application/json" },
                                  body: JSON.stringify({ tenantId, recommendationId: rec.id }),
                                })
                                if (!res.ok) throw new Error(await res.text())
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              setIsSubmitting(true)
                              try {
                                const res = await fetch("/app/api/perks/recommendations/dismiss", {
                                  method: "POST",
                                  headers: { "content-type": "application/json" },
                                  body: JSON.stringify({ tenantId, recommendationId: rec.id }),
                                })
