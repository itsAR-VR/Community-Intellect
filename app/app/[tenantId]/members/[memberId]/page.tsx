...
export default function MemberProfilePage() {
  ...
  const facts = mockFacts.filter((f) => f.memberId === memberId)
  const signals = mockSignals.filter((s) => s.memberId === memberId)
  const opportunities = mockOpportunities.filter((o) => o.memberId === memberId && !o.dismissed)
  const drafts = mockDrafts.filter((d) => d.memberId === memberId)
  const introSuggestions = mockIntroSuggestions.filter(
    (i) => (i.memberAId === memberId || i.memberBId === memberId) && !i.dismissed,
  )
  const introRecords = mockIntroRecords.filter((i) => i.memberAId === memberId || i.memberBId === memberId)
  const perkRecommendations = mockPerkRecommendations.filter((p) => p.memberId === memberId && !p.dismissed)
  const outcomes = mockOutcomeFeedback.filter((o) => o.memberId === memberId)
                  ...
                  <div className="space-y-3">
                    {introSuggestions.length === 0 ? (
                      ...
                      introSuggestions.map((suggestion) => {
                        ...
                        const otherMember = mockMembers.find((m) => m.id === otherMemberId)
