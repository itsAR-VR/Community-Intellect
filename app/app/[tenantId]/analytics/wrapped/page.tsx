...
import { mockMembers, mockWrapped } from "@/lib/mock-data"
...
export default function WrappedPage() {
  ...
  const wrapped = mockWrapped.find((w) => w.memberId === selectedMemberId)
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {wrapped.startSnapshot.goals.map((goal, i) => (
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {wrapped.startSnapshot.bottlenecks.map((b, i) => (
              ...
              <div className="flex items-center gap-2">
                ...
                <RiskBadge tier={wrapped.startSnapshot.riskTier} />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {wrapped.currentSnapshot.goals.map((goal, i) => (
                    <Badge key={i} variant="outline">
                      {goal}
                    </Badge>
                  ))}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {wrapped.currentSnapshot.bottlenecks.map((b, i) => (
                    <Badge key={i} variant="secondary">
                      {b}
                    </Badge>
                  ))}
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm text-muted-foreground">Risk Level:</span>
                <RiskBadge tier={wrapped.currentSnapshot.riskTier} />
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-5">
                <div className="text-center p-4 rounded-lg bg-muted">
                  ...
                  <p className="text-3xl font-bold">{wrapped.highlights.introsReceived}</p>
                  <p className="text-sm text-muted-foreground">Intros Received</p>
                </div>
                <div className="text-center p-4 rounded-lg bg-muted">
                  <Gift className="h-8 w-8 mx-auto mb-2 text-primary" />
                  <p className="text-3xl font-bold">{wrapped.highlights.perksUsed}</p>
                  <p className="text-sm text-muted-foreground">Perks Used</p>
                </div>
                <div className="text-center p-4 rounded-lg bg-muted">
                  <Calendar className="h-8 w-8 mx-auto mb-2 text-primary" />
                  <p className="text-3xl font-bold">{wrapped.highlights.eventsAttended}</p>
                  <p className="text-sm text-muted-foreground">Events Attended</p>
                </div>
                <div className="text-center p-4 rounded-lg bg-muted">
                  <BookOpen className="h-8 w-8 mx-auto mb-2 text-primary" />
                  <p className="text-3xl font-bold">{wrapped.highlights.resourcesAccessed}</p>
                  <p className="text-sm text-muted-foreground">Resources Accessed</p>
                </div>
                <div className="text-center p-4 rounded-lg bg-muted">
                  {wrapped.highlights.sentimentTrend === "improving" ? (
                    <TrendingUp className="h-8 w-8 mx-auto mb-2 text-success" />
                  ) : wrapped.highlights.sentimentTrend === "declining" ? (
                    <TrendingDown className="h-8 w-8 mx-auto mb-2 text-destructive" />
                  ) : (
                    <BarChart3 className="h-8 w-8 mx-auto mb-2 text-muted-foreground" />
                  )}
                  <p className="text-lg font-bold capitalize">{wrapped.highlights.sentimentTrend}</p>
