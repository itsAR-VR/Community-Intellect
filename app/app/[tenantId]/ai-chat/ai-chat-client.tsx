...
import type { ChatMessage, ChatThread, Member, TenantId } from "@/lib/types"
...
export function AIChatClient({
  ...
  const [selectedThread, setSelectedThread] = React.useState<string | null>(initialThreads[0]?.id ?? null)
  ...
  const handleSend = async () => {
    ...
    setIsSending(true)
    try {
      const history: ChatMessage[] = currentThread?.messages ?? []
      ...
      const res = await fetch("/app/api/ai/chat", {
        ...
        body: JSON.stringify({
          ...
          context: context === "member" ? { type: "member", memberId: selectedMember ?? undefined } : { type: "club" },
          messages: nextMessages,
        }),
      })
      if (!res.ok) throw new Error(await res.text())
      const json = (await res.json()) as { thread: ChatThread }

      setThreads((prev) => {
        const exists = prev.some((t) => t.id === json.thread.id)
        if (exists) return prev.map((t) => (t.id === json.thread.id ? json.thread : t))
        return [json.thread, ...prev]
      })
      setSelectedThread(json.thread.id)
    } catch (e) {
      toast({ title: "Chat failed", description: e instanceof Error ? e.message : "Unknown error" })
    } finally {
      setIsSending(false)
    }
  }
                ...
                {currentThread.messages.map((message) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {message.evidence?.memberIds?.length ? (
        ...
        <div className="p-3 md:p-4 border-t border-border">
          <div className="flex gap-2 max-w-2xl mx-auto">
            <Input
              ...
              className="flex-1"
            />
            <Button onClick={() => void handleSend()} disabled={!input.trim() || isSending} size="icon">
