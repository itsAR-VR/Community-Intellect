...
import { mockMembers, mockMastermindGroups, mockMonthlyAgendas, mockWorkshopPlans } from "@/lib/mock-data"
...
export default function ProgrammingPage() {
  ...
  const masterminds = mockMastermindGroups.filter((m) => m.tenantId === tenantId)
  const agenda = mockMonthlyAgendas.find((a) => a.tenantId === tenantId && a.month === monthString)
  const workshops = mockWorkshopPlans.filter((w) => w.tenantId === tenantId)
          ...
          {masterminds.length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {masterminds.map((group) => {
                ...
                const members = group.memberIds.map((id) => mockMembers.find((m) => m.id === id)).filter(Boolean)
