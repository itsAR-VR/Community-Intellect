...
import { mockMembers, mockIntroSuggestions, mockIntroRecords } from "@/lib/mock-data"
...
export default function IntrosPage() {
  ...
  const suggestions = mockIntroSuggestions.filter(
    (s) => !s.dismissed && (memberIds.includes(s.memberAId) || memberIds.includes(s.memberBId)),
  )

  const records = mockIntroRecords.filter((r) => memberIds.includes(r.memberAId) || memberIds.includes(r.memberBId))
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">Select from member list...</p>
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Textarea placeholder="Why should these members connect?" />
              </div>
              <Button className="w-full">Create Introduction</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
          ...
          {suggestions.length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {suggestions.map((suggestion) => {
                const memberA = mockMembers.find((m) => m.id === suggestion.memberAId)
                const memberB = mockMembers.find((m) => m.id === suggestion.memberBId)
                  ...
                  <Card key={suggestion.id}>
                    <CardContent className="p-4">
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        <div>
                          <Link
                            href={`/app/${tenantId}/members/${memberA.id}`}
                        ...
                        <div className="text-right">
                          <Link
                            href={`/app/${tenantId}/members/${memberB.id}`}
                            className="font-medium text-sm hover:underline"
                          >
            ...
            <CardContent>
              {records.length === 0 ? (
                ...
                <div className="space-y-3">
                  {records.map((record) => {
                    const memberA = mockMembers.find((m) => m.id === record.memberAId)
                    const memberB = mockMembers.find((m) => m.id === record.memberBId)
                    if (!memberA || !memberB) return null
