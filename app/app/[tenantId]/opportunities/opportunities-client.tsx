...
import type { TenantId, SignalSource, Member, OpportunityItem } from "@/lib/types"
...
export function OpportunitiesClient({
  ...
  const [opportunitiesState, setOpportunitiesState] = React.useState<OpportunityItem[]>(initialOpportunities)
  const [isMutating, setIsMutating] = React.useState(false)

  const opportunities = opportunitiesState.filter((o) => {
  ...
  const selectedMember = selectedOpportunity ? members.find((m) => m.id === selectedOpportunity.memberId) : null

  const dismiss = async () => {
    if (!selectedOpportunity) return
    setIsMutating(true)
    try {
      const res = await fetch("/app/api/opportunities/dismiss", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ tenantId, opportunityId: selectedOpportunity.id }),
      })
      if (!res.ok) throw new Error(await res.text())
      setOpportunitiesState((prev) => prev.map((o) => (o.id === selectedOpportunity.id ? { ...o, dismissed: true } : o)))
      setSelectedOpp(null)
  ...
  const addToForcedSuccess = async () => {
    if (!selectedOpportunity) return
    setIsMutating(true)
    try {
      const res = await fetch("/app/api/forced-success/add-from-opportunity", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ tenantId, opportunityId: selectedOpportunity.id }),
      })
      if (!res.ok) throw new Error(await res.text())
      toast({ title: "Added to Forced Success" })
    } catch (e) {
      ...
      {opportunities.length === 0 ? (
        ...
        <div className="grid gap-4">
          {opportunities.map((opp) => {
            const member = members.find((m) => m.id === opp.memberId)
            ...
            <>
              ...
              <div className="mt-6 space-y-6">
                ...
                <div className="flex gap-2">
                  ...
                  <Button className="flex-1" onClick={() => void addToForcedSuccess()} disabled={isMutating}>
