...
export function MemberProfileClient({
  ...
  const updateStatus = async (next: MemberStatus) => {
    setIsUpdatingStatus(true)
    try {
      const res = await fetch("/app/api/members/set-status", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ memberId: member.id, status: next }),
      })
      if (res.status === 409) {
        const json = (await res.json().catch(() => null)) as any
        const missing = Array.isArray(json?.missing) ? json.missing.join(", ") : ""
        toast({ title: "Intake incomplete", description: missing || "Member cannot be marked Active yet." })
        return
      }
                ...
                <div className="flex-1 space-y-4">
                  <div className="flex items-center gap-3">
                    {canEdit ? (
                      <Select
                        value={status}
                        onValueChange={(v) => updateStatus(v as MemberStatus)}
