...
import type { Member, Survey } from "@/lib/types"
import { formatDistanceToNow } from "date-fns"

export function SurveysClient({ members, surveys }: { members: Member[]; surveys: Survey[] }) {
  ...
  const send = async (memberIds: string[]) => {
    ...
    try {
      const res = await fetch("/app/api/surveys/send", {
        ...
        body: JSON.stringify({ memberIds, cadence }),
      })
