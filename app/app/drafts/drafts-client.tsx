...
import type { MessageDraft, Member } from "@/lib/types"
import { formatDistanceToNow } from "date-fns"

export function DraftsClient({ members, drafts: initialDrafts }: { members: Member[]; drafts: MessageDraft[] }) {
  ...
  const regenerateDraft = async (draft: MessageDraft) => {
    try {
      const res = await fetch("/app/api/ai/draft-message", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ memberId: draft.memberId, actionType: draft.actionType }),
      })
      if (!res.ok) throw new Error(await res.text())
